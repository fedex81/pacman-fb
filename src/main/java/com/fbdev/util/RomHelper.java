...
public class RomHelper {
    ...
    private static void loadRom() {
        try {
            byte[] r6e = Files.readAllBytes(Paths.get(ROMS_FOLDER, "pacman.6e"));
            byte[] r6f = Files.readAllBytes(Paths.get(ROMS_FOLDER, "pacman.6f"));
            byte[] r6h = Files.readAllBytes(Paths.get(ROMS_FOLDER, "pacman.6h"));
            byte[] r6j = Files.readAllBytes(Paths.get(ROMS_FOLDER, "pacman.6j"));
            ...
            INSTANCE.crom = Files.readAllBytes(Paths.get(ROMS_FOLDER, "82s123.7f"));
            INSTANCE.palRom = Files.readAllBytes(Paths.get(ROMS_FOLDER, "82s126.4a"));
            INSTANCE.tileRom = Files.readAllBytes(Paths.get(ROMS_FOLDER, "pacman.5e"));
            INSTANCE.spriteRom = Files.readAllBytes(Paths.get(ROMS_FOLDER, "pacman.5f"));
            byte[] s1 = Files.readAllBytes(Paths.get(ROMS_FOLDER, "82s126.1m"));
            byte[] s2 = Files.readAllBytes(Paths.get(ROMS_FOLDER, "82s126.3m"));
